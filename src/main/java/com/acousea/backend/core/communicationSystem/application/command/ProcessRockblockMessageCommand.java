...
public class ProcessRockblockMessageCommand extends Command<RockBlockMessage, String> {
    ...
    public ApiResult<String> execute(RockBlockMessage message) throws InvalidPacketException {
        ...
        // Trigger the event
        NodeDevice nodeDevice = nodeDeviceRepository.findByNetworkAddress(communicationResponse.getRoutingChunk().sender()).orElseThrow(
        ...
        switch (communicationResponse.getOperationCode()) {
            ...
            case SUMMARY_REPORT:
                ...
                eventBus.publish(
                        new ReceivedSummaryReportEvent(
                                new ReceivedSummaryReportEvent.Payload(
                                        nodeDevice.getName()
                                )
                        )
                );
